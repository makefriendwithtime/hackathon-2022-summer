...
contract Airdrop is ERC20{
        ...
        for(uint i=0;i < _memberTotal;i++){
            ...
            if(memberTime > 0 && amount > 0){
                ...
                if(day > 0 && day.mod(_calTime) == 0){       
                    //池子剩余token，按比例空投算法
                    // amount = balance.mul(amount).mul(dropProportion).div(totalSupply).div(100);    
                    //池子持有stktoken，按比例空投算法
                    amount = amount.mul(dropProportion).div(100);   
    ...
    function unlockLeaseMargin(
        ...
        if(period > 0 && _leaseDate.add(period * dayLen) < block.timestamp.div(24 * 60 * 60)){
